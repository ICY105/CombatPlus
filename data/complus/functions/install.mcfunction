...
#check DU
execute unless score $du.ver load matches 2010200.. run scoreboard players set $install complus_cooldown 0
execute unless score $du.ver load matches 2010200.. run tellraw @a [{"text":"Error: Combat+ requires Datapack Utilities version +2.1.2. You can download that [here].\n","color":"red","clickEvent":{"action":"open_url","value":"https://github.com/ImCoolYeah105/Datapack-Utilities/releases"}}]

#print install message
execute if score $install complus_cooldown matches 1 run tellraw @a [{"text":"[Loaded Combat+ v1.1.2]","color":"dark_green"}]
